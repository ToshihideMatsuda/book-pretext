...
let currentPage = 0
const pageCursors: LayoutCursor[] = []
let isAnimating = false
const layerPools = new Map<HTMLElement, LayerPool>()

function getPool(layer: HTMLElement): LayerPool {
  let p = layerPools.get(layer)
  if (!p) { p = { lines: [], headlines: [], dropCap: null }; layerPools.set(layer, p) }
  return p
}

function syncPool<T extends HTMLElement>(layer: HTMLElement, pool: T[], count: number, create: () => T): void {
  while (pool.length < count) { const el = create(); layer.appendChild(el); pool.push(el) }
...
function fitHeadline(maxWidth: number, maxHeight: number, maxSize: number): { fontSize: number; lines: PositionedLine[] } {
  const key = `${maxWidth}x${maxHeight}x${maxSize}`
  ...
  while (lo <= hi) {
    ...
    walkLineRanges(prep, maxWidth, line => { lineCount++; if (line.end.graphemeIndex !== 0) breaksWord = true })
    if (!breaksWord && lineCount * lh <= maxHeight) {
      best = size
      bestLines = layoutWithLines(prep, maxWidth, lh).lines.map((l, i) => ({ x: 0, y: i * lh, text: l.text, width: l.width }))
      lo = size + 1
    } else { hi = size - 1 }
  }
...
function isSameCursor(a: LayoutCursor, b: LayoutCursor): boolean {
  return a.segmentIndex === b.segmentIndex && a.graphemeIndex === b.graphemeIndex
}

function renderPageToLayer(layer: HTMLElement, startCursor: LayoutCursor, pageIndex: number): RenderedPage {
  ...
  const maxHlHeight = Math.floor(ph * (isNarrow ? 0.2 : 0.24))
  const maxHlSize = isNarrow ? 38 : 92
  const { fontSize: hlSize, lines: hlLines } = fitHeadline(headlineWidth, maxHlHeight, maxHlSize)
  ...
  const columnCount = pw > 1000 ? 3 : pw > 640 ? 2 : 1
  const columnWidth = Math.floor((Math.min(pw, 1500) - gutter * 2 - colGap * (columnCount - 1)) / columnCount)
  const contentLeft = Math.round((pw - (columnCount * columnWidth + (columnCount - 1) * colGap)) / 2)
  ...
  if (isFirstPage) {
    if (!pool.dropCap) {
      ...
      layer.appendChild(dc)
      pool.dropCap = dc
    }
    pool.dropCap.textContent = DROP_CAP_TEXT
    ...
    if (pool.dropCap) pool.dropCap.style.display = 'none'
  }

  // Layout columns
  const dropCapRect: RectObstacle | null = isFirstPage ? {
    x: contentLeft - 2, y: bodyTop - 2, w: DROP_CAP_TOTAL_W, h: DROP_CAP_LINES * BODY_LINE_HEIGHT + 2,
  } : null

  const allLines: PositionedLine[] = []
  let cursor = startCursor
  for (let ci = 0; ci < columnCount; ci++) {
    const colX = contentLeft + ci * (columnWidth + colGap)
    const rects: RectObstacle[] = ci === 0 && dropCapRect ? [dropCapRect] : []
    const result = layoutColumnSimple(preparedBody, cursor, colX, bodyTop, columnWidth, bodyHeight, BODY_LINE_HEIGHT, rects)
    allLines.push(...result.lines)
    cursor = result.cursor
  }

  // Render headline
  syncPool(layer, pool.headlines, hlLines.length, () => {
    const el = document.createElement('span'); el.className = 'headline-line'; return el
  })
  for (let i = 0; i < hlLines.length; i++) {
    const el = pool.headlines[i]!, line = hlLines[i]!
  ...
  for (let i = 0; i < allLines.length; i++) {
    const el = pool.lines[i]!, line = allLines[i]!
    el.textContent = line.text
...
// Initialize pages
const firstBodySeg = preparedBody.segments?.[0] ?? ''
const initialCursor: LayoutCursor = firstBodySeg === DROP_CAP_TEXT
  ? { segmentIndex: 1, graphemeIndex: 0 }
  : { segmentIndex: 0, graphemeIndex: 1 }

pageCursors.push(initialCursor)

const firstPage = renderPageToLayer(frontLayer, pageCursors[0]!, 0)
pageCursors.push(firstPage.cursor)
syncLayerStack()
...
function updatePageCounter(): void {
  ...
  if (el) el.textContent = String(currentPage + 1)
}
...
function goNextPage(): void {
  ...
  const nextPage = renderPageToLayer(backLayer, nextCursor, currentPage + 1)
  if (!nextPage.hasBodyContent) {
    clearLayer(backLayer)
    backLayer.classList.add('page-hidden')
    return
  }
  pageCursors[currentPage + 2] = nextPage.cursor
  ...
  frontLayer.addEventListener('animationend', () => {
...
function goPrevPage(): void {
  ...
  isAnimating = true
  renderPageToLayer(backLayer, pageCursors[currentPage - 1]!, currentPage - 1)
  setAnimatingLayerStack()
  backLayer.classList.remove('page-hidden')
...
window.addEventListener('click', (e: MouseEvent) => {
  ...
  if (target.closest('#nav-prev, #nav-next')) return
  if (e.clientX > window.innerWidth * 0.5) goNextPage()
  else goPrevPage()
})

window.addEventListener('keydown', (e: KeyboardEvent) => {
  if (e.key === 'ArrowRight' || e.key === ' ') { e.preventDefault(); goNextPage() }
  if (e.key === 'ArrowLeft') { e.preventDefault(); goPrevPage() }
})

let touchStartX = 0
window.addEventListener('touchstart', (e: TouchEvent) => { touchStartX = e.touches[0]!.clientX }, { passive: true })
window.addEventListener('touchend', (e: TouchEvent) => {
  const dx = e.changedTouches[0]!.clientX - touchStartX
  if (Math.abs(dx) > 50) { if (dx < 0) goNextPage(); else goPrevPage() }
}, { passive: true })

window.addEventListener('resize', () => {
  if (isAnimating) return
  cachedHeadlineKey = ''
  const currentPageRender = renderPageToLayer(frontLayer, pageCursors[currentPage]!, currentPage)
  pageCursors[currentPage + 1] = currentPageRender.cursor
  clearLayer(backLayer)
  backLayer.classList.add('page-hidden')
})
